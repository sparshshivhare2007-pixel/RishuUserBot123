...
from RishuUserBot.database.pmpermitdb import get_approved_users, pm_guard
import RishuUserBot.database.pmpermitdb as RishuUserBot
...
async def pmguard(client, message):
    ...
    if not arg:
        ...
        return
    await RishuUserBot.set_limit(int(arg))
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await RishuUserBot.set_block_message(RishuUserBot.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await RishuUserBot.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    await RishuUserBot.allow_user(chat_id)
...
async def deny(client, message):
    chat_id = message.chat.id
    await RishuUserBot.deny_user(chat_id)
