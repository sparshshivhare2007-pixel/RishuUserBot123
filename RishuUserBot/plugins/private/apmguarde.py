...
async def pmguard(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "off":
        await RishuUserBot.set_pm(False)
        await message.edit("**PM Guard Deactivated**")
    if arg == "on":
        await RishuUserBot.set_pm(True)
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await RishuUserBot.set_permit_message(RishuUserBot.PMPERMIT_MESSAGE)
        await message.edit("**Anti_PM message set to default**.")
        return
    await RishuUserBot.set_permit_message(f"`{arg}`")
